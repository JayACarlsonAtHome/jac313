...
void print_test_purpose(size_t total, size_t keeper_n, size_t db_n) {
    ...
    std::cout << " Plan: " << format_locale_int(total) << " events × "
              << format_locale_int(RUNS) << " runs, "
              << format_locale_int(keeper_n) << " keeper + "
              << format_locale_int(db_n) << " database flags\n\n";
}
    ...
    for (size_t run = 0; run < RUNS; ++run) {
        ...
        if (!status) {
            ...
            if (is_last) {
                ...
                std::cout << "  jText main rows:  " << format_locale_int(file_rows)
                          << " (expected " << format_locale_int(keeper_n) << ")\n";
                std::cout << "  SQLite main rows: " << format_locale_int(sql_rows)
                ...
                std::cout << "  PASS — " << format_locale_int(keeper_n + db_n)
                          << " / " << format_locale_int(TOTAL) << " persisted\n\n";
            }
        }
    }
