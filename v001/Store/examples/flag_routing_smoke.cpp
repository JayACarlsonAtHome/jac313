...
#include <jac313/Store/v001/headers/persistence/Sqlite.hpp>
...
jac313::Store::v001::PersistedEvent make_event(
...
int main() {
    ...
    auto file_sink = std::make_unique<jac313::Store::v001::BinaryEventSink>(
        out.string(), 1, 1, jac313::Store::v001::PersistMode::KeeperOnly, 1 << 20);
    auto sql_sink = std::make_unique<jac313::Store::v001::SqlEventSink>(
        out.string(), 1, 1, jac313::Store::v001::PersistMode::DatabaseOnly, false);

    auto routing_sink = std::make_unique<jac313::Store::v001::FlagRoutingEventSink>(
        std::move(file_sink), std::move(sql_sink));
    jac313::Store::v001::DoubleBufferedWriter writer(std::move(routing_sink), 4);
    ...
    jac313::Store::v001::Sqlite db(db_path);
    ...
    std::cout << "jac313::Store::v001 FlagRouting persistence smoke OK\n";
    return EXIT_SUCCESS;
}