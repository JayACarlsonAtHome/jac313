...
├── Qlite/                    # SQLite wrapper umbrella  (jac313::Qlite::v001)
├── jText/                    # structured-text umbrella (jac313::jText::v001)
├── Store/                    # time-series logging store (jac313::Store::v001)
...
Every package lives under `jac313::<Name>::v001`. The `v001` suffix is a **contract, not a
...
`jac::ts_store::inline_v001`) keep working alongside the umbrellas.
...
The umbrella lets us point new work at `jac313::*::v001` immediately, move implementation across
...
flowchart TB
    ...
    subgraph jac313 [jac313 monorepo]
        subgraph umbrellas [Umbrella packages v001]
            QL[jac313::Qlite::v001]
            JT[jac313::jText::v001]
            ST[jac313::Store::v001]
        end
        subgraph modules [Optional C++23 modules]
            QM[jac313.qlite.v001]
            JM[jac313.jtext.v001]
            SM[jac313.store.v001]
        end
        CLI[jac313_test_cli]
    end
    APP --> umbrellas
...
- **Versioned namespaces** — `::v001` is a contract, not a comment.
