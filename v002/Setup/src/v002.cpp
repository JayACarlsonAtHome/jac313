...
bool write_setup_script(const fs::path& source_dir,
    ...
    out << "# Each step runs independently; failures are reported at the end, not aborted on,\n";
    out << "# so one bad recipe can no longer hide the installs after it.\n";
    out << "set -uo pipefail\n\n";
    out << "__jac_fail=0\n\n";

    for (const ProvisionStep& step : plan.steps) {
        const std::string label = step.desc.empty() ? step.component : step.desc;
        out << "echo; echo \"=== " << label << "  [" << step.component << "] ===\"\n";
        out << "if " << step.command << "\n";
        out << "then echo \"  [ok]   " << step.component << "\"\n";
        out << "else echo \"  [FAIL] " << step.component << "\"; __jac_fail=1\n";
        out << "fi\n\n";
    }

    for (const std::string& comp : plan.unsupported) {
        out << "echo \"  [FAIL] " << comp << " — needed but recipes.conf has no entry for family '"
            << plan.family << "'; install it manually, or add a recipe.\"; __jac_fail=1\n";
    }
    ...
    out << "echo\n";
    out << "if [ \"$__jac_fail\" -eq 0 ]; then\n";
    out << "  echo 'Prerequisites installed. Re-run sensing to refresh readiness:'\n";
    out << "  echo '  " << reinvoke_hint << "'\n";
    out << "else\n";
    out << "  echo 'One or more steps FAILED above — fix those, then re-run this script.'\n";
    out << "fi\n";
    out << "exit \"$__jac_fail\"\n";
    out.close();
...
// ---- provisioning: in-process execution -----------------------------------------

ProvisionPlan plan_for_components(const fs::path& source_dir,
                                  const std::string& family,
                                  const std::vector<std::string>& components) {
    ProvisionPlan plan;
    plan.family = family;

    const std::vector<ProvisionRecipe> recipes = load_recipes(source_dir);
    auto find_recipe = [&](const std::string& name) -> const ProvisionRecipe* {
        for (const ProvisionRecipe& r : recipes) {
            if (r.component == name) {
                return &r;
            }
        }
    ...
    for (const std::string& comp : components) {
        const ProvisionRecipe* r = find_recipe(comp);
        if (r == nullptr) {
            plan.unsupported.push_back(comp); // bootstrap asked for an unknown component
            continue;
        }
        // Idempotent: if the recipe knows how to test itself and the test passes, skip it.
        if (!r->check.empty()) {
            const CaptureResult c = run_capture(r->check, 20);
            if (c.started && c.exit_code == 0) {
                continue;
        ...
        const auto it = r->install_by_family.find(family);
        if (it == r->install_by_family.end() || it->second.empty()) {
            plan.unsupported.push_back(comp);
            continue;
        }
        plan.steps.push_back({r->component, r->desc, it->second});
    }
...
// Run one install command with stdio INHERITED, so sudo can prompt on the terminal and
// the package manager's progress is visible. Returns the command's exit code (or -1 if
// it could not be launched). Unlike run_capture (which hides output for sensing), this
// is for the user-facing install path.
int run_inherited(const std::string& command_line) {
    const pid_t pid = ::fork();
    if (pid < 0) {
        return -1;
    }
...
ExecReport execute_plan(const ProvisionPlan& plan, const ExecOptions& opts) {
    ExecReport report;
    report.total = static_cast<int>(plan.steps.size());
    report.unsupported = plan.unsupported;

    std::cout << "\njac313 provisioning plan (family: "
              << (plan.family.empty() ? "unknown" : plan.family) << ")\n";
    if (plan.steps.empty()) {
        std::cout << "  (nothing to install — prerequisites already satisfied)\n";
    }
    for (std::size_t i = 0; i < plan.steps.size(); ++i) {
        const ProvisionStep& s = plan.steps[i];
        std::cout << "  " << (i + 1) << ") " << (s.desc.empty() ? s.component : s.desc)
                  << "  [" << s.component << "]\n";
        std::cout << "       $ " << s.command << "\n";
    }
    for (const std::string& comp : plan.unsupported) {
        std::cout << "  !  " << comp << " — no recipe for family '" << plan.family
                  << "'; install it manually.\n";
    }

    if (plan.steps.empty()) {
        return report; // nothing to do; unsupported (if any) already reported
    }
    ...
    if (!opts.assume_yes) {
        std::cout << "\nThese commands modify your system and may need sudo. Proceed? [y/N] "
                  << std::flush;
        std::string answer;
        if (!std::getline(std::cin, answer)) {
            answer.clear();
        }
        const bool yes = answer == "y" || answer == "Y" || answer == "yes" || answer == "YES";
        if (!yes) {
    ...
    for (std::size_t i = 0; i < plan.steps.size(); ++i) {
        const ProvisionStep& s = plan.steps[i];
        std::cout << "\n=== [" << (i + 1) << "/" << plan.steps.size() << "] " << s.component
                  << " ===\n" << std::flush;
        const int code = run_inherited(s.command);
        if (code == 0) {
            std::cout << "  [ok]   " << s.component << "\n";
            ++report.ok;
        } else {
            std::cout << "  [FAIL] " << s.component << " (exit " << code << ")\n";
            ++report.failed;
            report.failed_components.push_back(s.component);
        }
    }

    std::cout << "\nProvisioning summary: " << report.ok << " ok, " << report.failed
              << " failed, of " << report.total << " step(s).\n";
    if (report.failed == 0) {
        std::cout << "All steps succeeded.\n";
    } else {
        std::cout << "Failed: ";
        for (std::size_t i = 0; i < report.failed_components.size(); ++i) {
            std::cout << (i ? ", " : "") << report.failed_components[i];
        }
