// jac313_clean_slate — reset results.db to a blank slate: the schema and the testType enum are kept;
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else { std::cerr << "jac313_clean_slate: unknown arg '" << a << "' (try --help)\n"; return 2; }
    }
    if (dry && yes)   { std::cerr << "jac313_clean_slate: --dry-run and --yes can't be used together.\n"; return 2; }
    if (!dry && !yes) { std::cerr << "jac313_clean_slate: pass --dry-run (preview) or --yes (do it).\n"; return 2; }

    const fs::path db_path = src / "test-summary" / "results.db";
    std::error_code ec;
    if (!fs::exists(db_path, ec)) { std::cout << "clean-slate: no results.db at " << db_path << " (already blank).\n"; return 0; }
