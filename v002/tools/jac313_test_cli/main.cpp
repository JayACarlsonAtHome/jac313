...
void write_package_versions(jac313::Qlite::v002::Sqlite& db, std::int64_t gid,
                            const fs::path& src, const std::string& version) {
    struct Pkg { const char* name; const char* file; };
    static const Pkg pkgs[] = {
        {"Store", "Store/include/jac313/Store/v002/headers/ts_store_config.hpp"},
        {"Qlite", "Qlite/include/jac313/Qlite/v002/Sqlite.body.hpp"},
...
std::optional<std::int64_t> pin_current_host(jac313::Qlite::v002::Sqlite& db, const GlobalOptions& global,
...
inline std::string summary_context_line(const GlobalOptions& global) {
    ...
    try {
        jac313::Qlite::v002::Sqlite db(JAC313_RESULTS_DB);
                      ...
                      const std::optional<fs::path>& report_path)
{
    ...
    // Create run early so we can log live status events to runEvent table (visible in DataGrip)
    std::int64_t run_id = 0;
    const fs::path db_path = JAC313_RESULTS_DB;
    try {
        jac313::Qlite::v002::Sqlite db(db_path.string());
        run_id = cli_begin_run(db, global);
        jac313::results::log_run_event(db, run_id, "", "run_started", "ctest/smoke run starting");
    } catch (...) {}

    const auto results = run_tests(tests, opts, global.verbose, run_id, db_path);
    if (results.empty()) {
        return 1;
    }
    ...
    if (run_id > 0) {
        try {
            jac313::Qlite::v002::Sqlite db(db_path.string());
            jac313::results::log_run_event(db, run_id, "", "run_ended", "ctest/smoke run complete");
        } catch (...) {}
    }
...
int run_report_command(const fs::path& source_dir) {
    ...
    try {
        ...
        jac313::Qlite::v002::Sqlite db(db_path.string(), SQLITE_OPEN_READONLY);
...
// If a run_id is provided (>0), reuse it (for live status logging + testRun rows on same run).
// Otherwise fall back to creating a new one via cli_begin_run.
void record_ctest_results(const fs::path& source_dir, const fs::path& build_dir,
                          const std::vector<TestResult>& results, std::int64_t run_id) {
    ...
    try {
        ...
        if (run_id == 0) {
            GlobalOptions rec; rec.source_dir = source_dir;
            run_id = cli_begin_run(db, rec);
        } else {
            // Ensure schema and that the run row exists (it should from early creation)
            jac313::results::ensure_schema(db);
        }
...
// If a run_id is provided (>0), reuse it.
void record_matrix_results(const fs::path& source_dir, const fs::path& build_dir,
                           ...
                           const std::string& modules, const std::string& size, std::int64_t run_id = 0) {
    if (results.empty()) return;
    const fs::path db_path = JAC313_RESULTS_DB;
    std::error_code ec;
    if (!fs::exists(db_path.parent_path(), ec)) return;
...
int run_everything_command(const GlobalOptions& global) {
    ...
    try {
        jac313::Qlite::v002::Sqlite db(SHARED_DB);
