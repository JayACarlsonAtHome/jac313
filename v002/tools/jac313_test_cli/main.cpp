...
std::int64_t cli_begin_run(jac313::Qlite::v002::Sqlite& db);  // forward for early run creation in run_tests_command for live logging
...
static const std::string JAC313_VERSION = "v002";
                       ...
                       const PresetOptions& preset)
{
    ...
    if (preset.bench) {
        ...
        for (const auto& cc : preset_ccs) {
            ...
            emit("\"$BENCH\" --suite --db " + JAC313_RESULTS_DB + " --jtext-ver v002.004");
        }
...
int run_everything_command(const GlobalOptions& global) {
    ...
    for (const std::string& cc : {gcc_label, clang_label}) {
        ...
        step("bench " + cc, dir + "/Store/tests/matrix/jac313_store_bench --suite "
                            "--db " + SHARED_DB + " --jtext-ver v002.004");
    }
