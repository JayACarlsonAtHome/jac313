...
(e.g. `v002/test-summary/`). The top-level `test-summary/README.md` (in the shared results
...
- Per compiler (e.g. gcc15/clang): **median ops/sec** + low–high band + output size.
- Always read the **median + band**. Different machines show different absolutes (e.g. some
  hardware shows non-durable ~4–9M, durable ~0.4–1.2M depending on scale/backend).
...
|--------|------------|-------|
| **In-memory ceiling** (non-durable, low flags) | hot loop, no sink attached | Often 4–9M+ ops/sec depending on hardware (e.g. ~5M on some recent CPUs, ~9M on others); σ low. |
| **Durable sustained** | hot loop throttled by disk flush | Typically 0.4–1.2M ops/sec (varies by backend, event scale, and machine). Binary often highest, then jText/SQL depending on size. |
...
## Legacy notes (per-test logs)

Older per-test logs (`test_00N` in the functional matrix) may still contain "ops/sec" lines,
but **throughput is no longer sourced from them**. All current throughput data comes from the
dedicated `store_bench --suite` (visible in `test-summary/bench/` reports per machine).

`test_006` (if referenced in old logs) was a tail-reader concurrency/correctness stress test,
not throughput. Functional/correctness validation (ctest + smoke) remains separate.

See the live structured reports under `test-summary/` (per version) and [Benchmarks.md](Benchmarks.md).
