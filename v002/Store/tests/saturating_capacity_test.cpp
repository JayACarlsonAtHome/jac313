...
int main() {
    ...
    check(std::mul_sat<std::size_t>(5, 20) == 100, "mul_sat exact 5*20");
    check(std::add_sat<std::size_t>(100, 50) == 150, "add_sat exact 100+50");

    // overflow saturates to SIZE_MAX (the safety property)
    check(std::mul_sat<std::size_t>(MAX / 2 + 1, 4) == MAX, "mul_sat overflow saturates");
    check(std::mul_sat<std::size_t>(MAX, MAX) == MAX, "mul_sat MAX*MAX saturates");
    check(std::add_sat<std::size_t>(MAX, std::size_t{1} << 24) == MAX, "add_sat overflow saturates");
