...
int main() {
    const fs::path out = fs::temp_directory_path() / "jac313_store_jtext_smoke";

    for (const auto& suffix : {"", "_Ints", "_Floats"}) {
        fs::remove(out.string() + suffix + ".jtext");
    }

    auto sink = std::make_unique<jac313::Store::v002::JTextEventSink>(
    ...
    if (!fs::exists(out.string() + ".jtext")) {
        std::cerr << "expected main .jtext file\n";
        return EXIT_FAILURE;
    }

    std::cout << "jac313::Store::v002 jText persistence smoke OK\n";
    return EXIT_SUCCESS;
}