...
Implementation lives **in this package** under `jac313::Store::v002` (migrated from the legacy `jac::ts_store::inline_v002` repo — no sibling checkout needed).
...
using Config = jac313::Store::v002::ts_store_config<true, 6, 20, 43, 9, 6, false>;
...
`jac313::Store::v002::version()` returns the package's `"major.minor"` version (`"v002.001"`
...
| Path | Purpose |
|------|---------|
| `include/jac313/Store/v002.hpp` | Public entry |
| `include/jac313/Store/v002/headers/` | Core store implementation (header-only) |
| `modules/jac313.store.v002.cppm` | Optional C++23 module |
...
cmake --build build
./build/examples/jac313_store_v002_example
```
...
`-DJAC313_BUILD_MODULES=ON` builds the textual-wrapper `jac313.store.v002` module
